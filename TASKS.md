...
| 6.4 | AI Conversation Loop | Цикл: История -> LLM -> Ответ. | Добавлен `services/api/src/services/conversation.py` с LangChain tool-calling циклом и сохранением `Message`; обновлен `services/api/src/services/ai.py`, тест `services/api/tests/test_conversation_service.py`. Ruff: ok. Pytest: ok. Commit: TBD. | 6.3, 3.5 | [x] |
