...
| 11.3 | Admin: Core Package Import | Исправить импорт `core.database` в Streamlit runtime. | Добавлен `services/admin/src/core/__init__.py`, чтобы `core.database` корректно импортировался в Streamlit runtime. Pytest admin: ok. Commit: PENDING. | 11.1 | [x] |
