...
| ID | Task | Description | Context | Deps | Status |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 12.1 | GitHub Actions CI/CD | Добавить GitHub Actions pipeline: CI на PR/push (ruff, pytest, docker compose config) и CD на `main` для сборки/push Docker images в GHCR. | Добавлены `.github/workflows/ci.yml` и `.github/workflows/cd.yml`: CI гоняет `uv sync --frozen`, `ruff`, `pytest`, `docker compose config`; CD публикует образы `api/admin/worker/bot/migrate` в `GHCR` на push в `main`. Обновлен `README.md`, добавлен тест `services/api/tests/test_github_workflows.py`. Ruff: ok. Pytest: ok (118 passed). Commit: 0fee3b0. | 9.1, 9.2, 9.3 | [x] |
| 12.2 | Bot Runtime Log Fix | После CI/CD устранить текущую проблему в логах `bot` и перепроверить runtime. | В работе. | 12.1 | [/] |
| 12.3 | API: Webhook Error Handling | Обработать ошибки LLM/webhook без `500`, чтобы Telegram не ретраил update бесконечно. | `services/api/src/services/telegram_webhook.py` теперь перехватывает `RateLimitError`/`APIStatusError`/`APIError`, делает `session.rollback()`, отправляет пользователю fallback `Сервис перегружен...` и возвращает `status=deferred` вместо `500`; также обновлено создание `aiogram.Bot` через `DefaultBotProperties`. Добавлен тест `services/api/tests/test_telegram_webhook_service.py` на graceful fallback. Ruff: ok. Pytest: ok (120 passed). `docker compose up -d --build api`: ok. | 10.7 | [x] |
| 12.4 | API: Rollback ORM Expiry Fix | Исправить `MissingGreenlet` после `session.rollback()` в Telegram webhook fallback flow. | В `services/api/src/services/telegram_webhook.py` сохранены `tenant.bot_token` и `tenant.system_prompt` в локальные переменные до возможного `session.rollback()`, чтобы не трогать expired ORM-атрибуты после rollback; расширен тест `services/api/tests/test_telegram_webhook_service.py` сценарием с “expired” `bot_token` после rollback. Ruff: ok. Pytest: ok (120 passed). `docker compose up -d --build api`: ok. | 12.3 | [x] |
