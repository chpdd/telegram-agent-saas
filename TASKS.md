...
| 6.2 | AI Service Interface | Обертка над OpenRouter. | Добавлен `services/api/src/services/ai.py` (LangChain v1 `ChatOpenAI` с памятью диалога по `conversation_id`), тест `services/api/tests/test_ai_service.py`. Ruff: ok. Pytest: ok. Commit: TBD. | 2.1 | [x] |
